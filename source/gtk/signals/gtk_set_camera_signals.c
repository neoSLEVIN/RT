...
void	gtk_set_camera_signals(t_rt *rt)
{
	...
	g_signal_connect(G_OBJECT(camera->display_width.change), "value-changed",
		G_CALLBACK(display_width_scale_moved), rt);
	g_signal_connect(G_OBJECT(camera->display_height.change), "value-changed",
		G_CALLBACK(display_height_scale_moved), rt);
	g_signal_connect(G_OBJECT(camera->fov.change), "value-changed",
		G_CALLBACK(fov_scale_moved), rt);
	g_signal_connect(G_OBJECT(camera->fps.change), "value-changed",
