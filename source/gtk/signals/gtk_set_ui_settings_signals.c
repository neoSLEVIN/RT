...
{
	g_signal_connect(G_OBJECT(settings->mirror.change), "value-changed",
		G_CALLBACK(mirror_scale_moved), rt);
	g_signal_connect(G_OBJECT(settings->step.change), "value-changed",
		G_CALLBACK(step_angle_scale_moved), rt);
	g_signal_connect(G_OBJECT(settings->angle.change), "value-changed",
		G_CALLBACK(step_angle_scale_moved), rt);
	...
	g_signal_connect(G_OBJECT(settings->blur.change), "value-changed",
		G_CALLBACK(filter_spin_button_changer), rt);
	g_signal_connect(G_OBJECT(settings->sepia.change), "value-changed",
		G_CALLBACK(filter_spin_button_changer), rt);
	g_signal_connect(G_OBJECT(settings->noise.change), "value-changed",
		G_CALLBACK(filter_spin_button_changer), rt);
}
