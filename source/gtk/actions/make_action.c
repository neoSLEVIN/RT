...
void	make_action(t_rt *rt)
{
	if (rt->info->space_key)
		move_cam_with_shape(rt, GDK_KEY_space);
	if (rt->info->c_key)
		move_cam_with_shape(rt, GDK_KEY_c);
	if (rt->info->w_key)
		move_cam_with_shape(rt, GDK_KEY_w);
	if (rt->info->s_key)
		move_cam_with_shape(rt, GDK_KEY_s);
	if (rt->info->a_key)
		move_cam_with_shape(rt, GDK_KEY_a);
	if (rt->info->d_key)
		move_cam_with_shape(rt, GDK_KEY_d);
	if (rt->info->q_key)
